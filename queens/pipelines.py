...
class QueensPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `queens`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM queens WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO queens (date, title, link, content)"
